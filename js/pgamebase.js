...
let score2 = 0; // Variable para llevar el puntaje
let vidaBoss = 100;
class Nivel1 extends Phaser.Scene /*NIVEL 1*/ {
    ...
    preload()/*NIVEL 1*/ {
        ...
        for (let i = 1; i <= 10; i++) {
            const key = `p2jum${i}`; // Cambié p2_ por Idle
            const path = `./img/prota2/Jump${i}.png`; // Cambié el nombre de la imagen a Idle{n}
            this.load.image(key, path);
        }
        //Fin Jugador 2
    ...
    create()/*NIVEL 1*/ {
        ...
        // Texto para mostrar el puntaje del jugador 1
        this.scoreText = this.add.text(100, 60, 'P1 - Shield: 0%', {
            fontSize: '45px',
            fill: '0x2f2f2f',
            fontFamily: 'Arial'
        });

        this.scoreText.setText(`P1 - Shield: ${score}%`); // Actualizar texto

        if (jugadores == 2){
            // Texto para mostrar el puntaje del jugador 2
            this.score2Text = this.add.text(550, 60, 'P2 - Shield: 0%', {
                fontSize: '45px',
                fill: '0x2f2f2f',
                fontFamily: 'Arial'
            });
        ...
        function collectOrb(player, orb) {
            // Determinar cuál personaje interactuó con el orbe
            if (player === this.character) {
                score += 10; // Sumar puntos al puntaje del jugador 1
                this.scoreText.setText(`Shield: ${score}%`); // Actualizar texto
                console.log("P1 Score: " + score);
            } else if (player === this.character2) {
                score2 += 10; // Sumar puntos al puntaje del jugador 2
                this.score2Text.setText(`Shield: ${score2}%`); // Actualizar texto
                console.log("P2 Score: " + score2);
            }
            contarOrbes++;
            this.orbsound.play({ volume: 0.3 });
            colliderOrb(orb); // Desactivar el orbe
        }
        ...
        this.anims.create({
            key: 'run2',
            frames: Array.from({ length: 8 }, (_, i) => ({ key: `p2run${i + 1}` })), // Los frames de reposo0 a reposo9
            frameRate: 10,
            repeat: -1  // Esto hará que la animación se repita indefinidamente
        });
        ...
        this.anims.create({
            key: 'jump2',
            frames: Array.from({ length: 10 }, (_, i) => ({ key: `p2jum${i + 1}` })), // Los frames de reposo0 a reposo9
            frameRate: 6,
            repeat: -1  // Esto hará que la animación se repita indefinidamente
        });
    ...
    update() {
        ...
        if (jugadores == 2) {
            ...
            } else if (this.teclasJugador2.derecha.isDown && this.teclasJugador2.arriba.isDown) {
                
                this.character2.body.setVelocityX(260);  // Mover a la izquierda mientras saltas
                this.character2.flipX = false;              // Invertir la dirección del jugador 1
                this.character2.anims.play('jump2', true);  // Animación de salto para el jugador 1
                
            } else if (this.teclasJugador2.izquierda.isDown && this.teclasJugador2.arriba.isDown) {
                
                 this.character2.body.setVelocityX(-260);  // Mover a la izquierda mientras saltas
                this.character2.flipX = true;              // Invertir la dirección del jugador 1
                this.character2.anims.play('jump2', true);  // Animación de salto para el jugador 1
        ...
        // Verificar si se recolectaron todos los orbes
        if (contarOrbes == 10) {
            this.musicalv1.stop();
...
class Nivel2 extends Phaser.Scene/*NIVEL 2*/ {
    ...
    preload()/*NIVEL 2*/ {
        ...
        for (let i = 1; i <= 10; i++) {
            const key = `p2dead${i}`; // Cambié p2_ por Idle
            const path = `./img/prota2/Dead${i}.png`; // Cambié el nombre de la imagen a Idle{n}
            this.load.image(key, path);
        }
        //Fin Jugador 2
    }
    ...
    create()/*NIVEL 2*/ {
        ...
        if (jugadores == 2){
            ...
            this.score2Text.setText(`P2 - Shield: ${score2}%`); // Actualizar texto
        }
        // Crear el piso
        ...
        function collectOrb(player, orb) {
            // Determinar cuál personaje interactuó con el orbe
            if (player === this.character) {
                score += 25; // Sumar puntos al puntaje del jugador 1                
                if (score >= 100) {
                    score = 100;
                }
                this.scoreText.setText(`P1 - Shield: ${score}%`); // Actualizar texto
                ...
                score2 += 25; // Sumar puntos al puntaje del jugador 2                
                if (score2 >= 100) {
                    score2 = 100;
                }
                this.score2Text.setText(`P2 - Shield: ${score2}%`); // Actualizar texto
        ...
        if (jugadores == 2) {
            this.character2 = this.physics.add.sprite(80, 490, 'player2-1').setScale(0.23);
            // this.character2.setSize(250,500);
            // this.character2.setOffset(200,30);
            this.physics.add.collider(this.orb, piso);
            ...
            this.physics.add.overlap(this.character2, this.orb, collectOrb, null, this);
            this.physics.add.overlap(this.character2, enemigo[0], makeDamage, null, this);
            this.physics.add.overlap(this.character2, enemigo[1], makeDamage, null, this);
            this.physics.add.overlap(this.character2, enemigo[2], makeDamage, null, this);
        ...
        this.physics.add.overlap(this.character, enemigo[2], makeDamage, null, this);

        //this.physics.add.overlap(this.character, enemigo[1], makeDamage, null, this);

        function makeDamage(player, enemy) {
            // Determinar a qué jugador afecta el daño
            let playerScore, playerInvulnerable, playerCharacter, deathAnimation;
        
            if (player === this.character) {
                playerScore = 'score';
                playerInvulnerable = 'invulnerable';
                playerCharacter = this.character;
                deathAnimation = 'dead'; // Animación de muerte para jugador 1
            } else if (player === this.character2 && jugadores === 2) {
                playerScore = 'score2';
                playerInvulnerable = 'invulnerable2';
                playerCharacter = this.character2;
                deathAnimation = 'dead2'; // Animación de muerte para jugador 2
            } else {
                return; // Si no es un jugador válido, salir de la función
            }
            ...
            // Reducir el puntaje del jugador correspondiente
            ...
            // Comprobar si el puntaje es menor o igual a cero y si el jugador es golpeado otra vez
            if ((playerScore === 'score' && score <= 0) || (playerScore === 'score2' && score2 <= 0)) {
                if ((playerScore === 'score' && score < 0) || (playerScore === 'score2' && score2 < 0)) {
                    // Si ya está en negativo o golpeado otra vez tras llegar a cero, muere
                    console.log(`Game Over para ${player === this.character ? "Jugador 1" : "Jugador 2"}`);
                    this.musicalv2.stop();
                    this.physics.pause();
                    isDead = true;
                    this.deadsound.play({ volume: 0.5 });
                    playerCharacter.setTint(0xAE445A); // Cambiar el color del personaje
                    playerCharacter.anims.play(deathAnimation, true); // Animación de muerte específica
        
                    // Reiniciar el juego o cambiar de escena tras el Game Over
                    this.time.addEvent({
                        delay: 2000,
                        loop: false,
                        callback: () => {
            ...
            console.log(`Score ${player === this.character ? "Jugador 1" : "Jugador 2"}: ${playerScore === 'score' ? score : score2}`);
            playerCharacter.setTint(0xff0000);
            enemy.setTint(0x2f2f2f);
        
            // Restaurar color después de un tiempo
            this.time.addEvent({
                delay: 500,
                callback: () => {
                    playerCharacter.clearTint();
                },
                loop: false
            });
            this.time.addEvent({
            ...
            this.time.addEvent({
                delay: 2500, // Tiempo de invulnerabilidad
                callback: () => {
                    this[playerInvulnerable] = false; // Desactivar invulnerabilidad
                },
                loop: false
            });
        
            // Actualizar texto de puntaje solo si existe el segundo jugador
            this.scoreText.setText(`P1 - Shield: ${score}%`); // Actualizar texto para jugador 1
            if (jugadores === 2) {
                this.score2Text.setText(`P2 - Shield: ${score2}%`); // Actualizar texto para jugador 2
            }
        }
        ...
        function makeDamage_1(player) {
            ...
            if (score2 < 0) {
                ...
                //this.characterObject.setPosition(this.characterObject.x, this.characterObject.y + 25);
                this.deadsound.play({ volume: 0.5 });
                this.character2.setTint(0xAE445A); // Cambio de color para mostrar que ha perdido
                this.character2.anims.play('dead2', true);
                this.time.addEvent({
                    delay: 2000,
                    loop: false,
                    callback: () => {
                ...
                this.time.addEvent({
                    delay: 500, // Esperar 500ms
                    callback: () => {
                        this.character2.clearTint(); // Restaurar el color original
                    },
                    loop: false // Solo se ejecuta una vez
                });
                this.time.addEvent({
        ...
        this.anims.create({
            key: 'dead2',
            frames: Array.from({ length: 10 }, (_, i) => ({ key: `p2dead${i + 1}` })), // Los frames de reposo0 a reposo9
            frameRate: 25,
            repeat: 0  // Esto hará que la animación se repita indefinidamente
        });
...
class Nivel3 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.baseURL = './';
        this.load.audio('jump', './audios/7-Jump.mp3');
        this.load.audio('kunai_sound', './audios/10-prota_kunai.mp3');
        ...
        this.load.image('kunai', './img/prota/Kunai.png');
        ...
        for (let i = 1; i <= 6; i++) {
            const key = `bossthrow${i}`; // Cambié p2_ por Idle
            const path = `./img/finalboss/Throw${i}.png`; // Cambié el nombre de la imagen a Idle{n}
            this.load.image(key, path);
        }
        ...
        for (let i = 1; i <= 6; i++) {
            const key = `bossjuthr${i}`; // Cambié p2_ por Idle
            const path = `./img/finalboss/JumpThrow${i}.png`; // Cambié el nombre de la imagen a Idle{n}
            this.load.image(key, path);
        }

        for (let i = 0; i <= 9; i++) {
            const key = `shuriken${i}`;
            const path = `./img/finalboss/Shuriken__${String(i).padStart(3, '0')}.png`;
            this.load.image(key, path);
        }
        for (let i = 0; i <= 9; i++) {
            const key = `deadBoss${i}`;
            const path = `./img/finalboss/Dead__${String(i).padStart(3, '0')}.png`;
            this.load.image(key, path);
        }
    ...
    create() {
        ...
        this.jumpsound = this.sound.add('jump', { loop: false });
        this.kunaisound = this.sound.add('kunai_sound', { loop: false });
        this.character = this.physics.add.sprite(80, 490, 'prota').setScale(0.22);
        ...
        // Crear un grupo de kunais
        this.kunais = this.physics.add.group();
        // Variable para controlar el tiempo entre lanzamientos
        this.kunaiCooldown = false; // Indica si se puede lanzar un kunai
        this.kunaiCooldownTime = 800; // Tiempo en milisegundos entre lanzamientos

        // Crear un grupo de shuriken
        this.shurikens = this.physics.add.group();
        // Variable para controlar el tiempo entre lanzamientos
        this.shurikenCooldown = false; // Indica si se puede lanzar un kunai
        this.shurikenCooldownTime = 800; // Tiempo en milisegundos entre lanzamientos
        ...
        this.anims.create({
            key: 'throwBoss',
            frames: Array.from({ length: 6 }, (_, i) => ({ key: `bossthrow${i + 1}` })), // Los frames de reposo0 a reposo9
            frameRate: 20,
            repeat: -1  // Esto hará que la animación se repita indefinidamente
        });
        ...
        this.anims.create({
            key: 'jumpthrowBoss',
            frames: Array.from({ length: 6 }, (_, i) => ({ key: `bossjuthr${i + 1}` })), // Los frames de reposo0 a reposo9
            frameRate: 20,
            repeat: -1  // Esto hará que la animación se repita indefinidamente
        });
        ...
        this.anims.create({
            key: 'deadBoss',
            frames: Array.from({ length: 6 }, (_, i) => ({ key: `deadBoss${i}` })), // Los frames de reposo0 a reposo9
            frameRate: 15,
            repeat: -1  // Esto hará que la animación se repita indefinidamente
        });
        ...
        this.anims.create({
            key: 'shuriken',
            frames: Array.from({ length: 9 }, (_, i) => ({ key: `shuriken${i}` })), // Los frames de reposo0 a reposo9
            frameRate: 25,
            repeat: -1  // Esto hará que la animación se repita indefinidamente
        });
        ...
        this.physics.add.collider(this.kunais, piso, (kunai) => {
            kunai.destroy(); // Eliminar kunais que colisionen con el piso
        });

        this.physics.add.collider(this.kunais, this.boss, (kunai) => {
            kunai.destroy(); // Eliminar kunais que colisionen con el piso
            this.time.addEvent({
                delay: 2000,
                loop: false,
    ...
    lanzarKunai() {
        // Solo permite lanzar kunai si no está en cooldown
        if (this.kunaiCooldown) return;

        // Crear un kunai en la posición del personaje
        const kunai = this.kunais.create(this.character.x, this.character.y, 'kunai').setScale(0.4);

        // Configurar la dirección del kunai
        if (this.character.flipX) {
            kunai.setVelocityX(-400); // Izquierda
            kunai.flipX = true;       // Voltea la imagen
        } else {
            kunai.setVelocityX(400);  // Derecha
        }

        // Configurar propiedades del kunai
        kunai.setGravityY(0);          // Sin efecto de gravedad
        kunai.body.allowGravity = false;
        kunai.setCollideWorldBounds(true);
        kunai.body.onWorldBounds = true;

        // Eliminar kunai si sale de la pantalla
        kunai.body.world.on('worldbounds', (body) => {
            if (body.gameObject === kunai) {
                kunai.destroy();
            }
        });

        // Activar el cooldown
        this.kunaiCooldown = true;
        this.kunaisound.play({ volume: 0.2 });
        // Temporizador para desactivar el cooldown
        this.time.delayedCall(this.kunaiCooldownTime, () => {
            this.kunaiCooldown = false; // Permitir lanzar de nuevo
        });
...
class MainMenu extends Phaser.Scene {
    ...
    create() {
        ...
        this.rectangulo.strokeRect(540, 480, 200, 60);
        this.rectangulo2.strokeRect(540, 550, 200, 60);
    
        this.selection = 1;
        this.enter = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ENTER);
    
        // ----------------------------
        // Agregar zonas interactivas táctiles
        // ----------------------------
    
        // Zona táctil para la primera opción (Play)
        const playZone = this.add.zone(640, 510, 200, 60).setInteractive(); // Coordenadas centrales de la opción "Play"
        ...
        // Zona táctil para la segunda opción (Controls)
        const controlsZone = this.add.zone(640, 580, 200, 60).setInteractive(); // Coordenadas centrales de la opción "Controls"
        controlsZone.on('pointerdown', () => {
            this.selection = 2;
            this.scene.start('controlsScene');
            this.music_mainmenu.stop();
        });
    
        // Opcional: Mostrar los límites de las zonas táctiles para depuración
        // Esto NO afecta tus rectángulos actuales
        // this.add.graphics()
        //     .lineStyle(2, 0xff0000)
        //     .strokeRect(playZone.x - 100, playZone.y - 30, 200, 60)
        //     .strokeRect(controlsZone.x - 100, controlsZone.y - 30, 200, 60);
    }
    

    update() {
        // Verifica si ENTER está presionado
        ...
        if (this.cursors.up.isDown) {
            this.selection = 1;
            console.log('Seleccionando Play');
            this.redrawRectangles(); // Redibujar los rectángulos
        } else if (this.cursors.down.isDown) {
            this.selection = 2;
            console.log('Seleccionando Controls');
            this.redrawRectangles(); // Redibujar los rectángulos
        }
    }
    
    // Método para redibujar los rectángulos según la selección actual
    redrawRectangles() {
        // Limpia los gráficos de los rectángulos
        this.rectangulo.clear();
        this.rectangulo2.clear();
    
        // Si la selección es 1, dibuja el primer rectángulo
        if (this.selection === 1) {
            this.rectangulo.lineStyle(4, 0x000000, 1); // Negro, opaco
            this.rectangulo.strokeRect(540, 480, 200, 60);
        }
    
        // Si la selección es 2, dibuja el segundo rectángulo
        if (this.selection === 2) {
            this.rectangulo2.lineStyle(4, 0x000000, 1); // Negro, opaco
            this.rectangulo2.strokeRect(540, 550, 200, 60);
        }
    }
...
class SelectPlayer extends Phaser.Scene {
    ...
    create() {
        ...
        // Inicializar gráficos de rectángulos
        this.rectangulo = this.add.graphics();
        this.rectangulo2 = this.add.graphics();

        // Dibujar el rectángulo inicial (opción 1 seleccionada por defecto)
        this.rectangulo.lineStyle(4, 0x000000, 1); // Borde negro
        this.rectangulo.strokeRect(540, 480, 200, 60); // Rectángulo de "1 Jugador"

        // Rectángulo de "2 Jugadores", inicialmente no visible
        this.rectangulo2.clear();

        // Configuración inicial de controles y selección
        this.cursors = this.input.keyboard.createCursorKeys();
        this.selection = 1; // Por defecto, selecciona "1 Jugador"
        this.enter = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ENTER);
        ...
        // Zona táctil para "1 Jugador"
        const onePlayerZone = this.add.zone(640, 510, 200, 60).setInteractive();
        onePlayerZone.on('pointerdown', () => {
            this.selection = 1;
            jugadores = 1; // Actualiza la variable global
            this.updateSelection(); // Actualiza los gráficos
            this.startGame(); // Inicia el juego
        });

        // Zona táctil para "2 Jugadores"
        const twoPlayerZone = this.add.zone(640, 580, 200, 60).setInteractive();
        twoPlayerZone.on('pointerdown', () => {
            this.selection = 2;
            jugadores = 2; // Actualiza la variable global
            this.updateSelection(); // Actualiza los gráficos
            this.startGame(); // Inicia el juego
        });
    }
    ...
    // Método para actualizar los gráficos de los rectángulos según la selección
    updateSelection() {
        // Limpia ambos rectángulos antes de redibujar
        this.rectangulo.clear();
        this.rectangulo2.clear();

        // Dibujar solo el rectángulo de la opción seleccionada
        if (this.selection === 1) {
            this.rectangulo.lineStyle(4, 0x000000, 1);
            this.rectangulo.strokeRect(540, 480, 200, 60); // Rectángulo de "1 Jugador"
        } else if (this.selection === 2) {
            this.rectangulo2.lineStyle(4, 0x000000, 1);
            this.rectangulo2.strokeRect(540, 550, 200, 60); // Rectángulo de "2 Jugadores"
        }
    }

    // Método para iniciar el juego
    startGame() {
        console.log(`Iniciando juego con ${jugadores} jugador(es).`);
...
class Controls extends Phaser.Scene {
    ...
    create() {
        ...
        this.rectangulo = this.add.graphics();
        this.redrawRectangle(); // Método para dibujar el rectángulo inicial

        // Habilitar entrada por teclado
        this.enter = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ENTER);

        // ----------------------------
        // Zona interactiva táctil
        // ----------------------------
        const backZone = this.add.zone(620, 575, 240, 60).setInteractive(); // Centrar en el rectángulo
        backZone.on('pointerdown', () => {
            this.scene.start('mainMenu'); // Regresar al menú principal
        });

        // Opcional: Depuración de la zona
        // this.add.graphics()
        //     .lineStyle(2, 0xff0000)
        //     .strokeRect(backZone.x - 120, backZone.y - 30, 240, 60);
    }

    update() {
        // Navegar de vuelta al menú principal con ENTER
    ...
    redrawRectangle() {
        // Limpia y dibuja el rectángulo
        this.rectangulo.clear();
        this.rectangulo.lineStyle(4, 0xffffff, 1); // Línea blanca
        this.rectangulo.strokeRect(500, 545, 240, 60); // Coordenadas y dimensiones del rectángulo
    }
}
...
const config = {
    ...
    scene: [MainMenu,Nivel1,  Nivel2, SelectPlayer, Nivel3,  Controls, EndGame, EndGame2],
    scale: {
